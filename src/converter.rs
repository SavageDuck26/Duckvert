...
/// Returns a path inside `dir` that doesn't already exist. When multiple
/// sources share a stem (e.g. `a/pic.png` and `b/pic.png` both landing in
/// one shared output folder), later files get `-2`, `-3`, ... suffixes
...
/// Path stem (no extension) for multi-page outputs like pdftoppm, which
/// appends `-1`, `-2`, ... to the base name (e.g. `name_converted-1.jpg`).
fn output_path_base(src: &Path, output_dir: Option<&Path>) -> PathBuf {
    let stem = src.file_stem().unwrap_or_default().to_string_lossy();
    let name = format!("{stem}_converted");
    match output_dir {
        Some(dir) => {
            fs::create_dir_all(dir).ok();
            unique_path(dir, &name)
...
fn convert_pdf_to_text(src: &Path, output_dir: Option<&Path>) -> Option<PathBuf> {
    ...
    for (_num, &obj_id) in &page_ids {
        if let Ok(content) = doc.extract_text(&[obj_id.0]) {
...
fn convert_pdf_to_image(src: &Path, target_fmt: &str, output_dir: Option<&Path>) -> Option<PathBuf> {
    // Formats pdftoppm can produce directly. Everything else falls back to ffmpeg.
    let fmt = match target_fmt {
        "jpg" | "jpeg" => "jpeg",
        "png" => "png",
        "tiff" | "tif" => "tiff",
        "bmp" => "bmp",
        "webp" | "ico" | "gif" => {
            println!("pdftoppm can't output {target_fmt}; using ffmpeg instead...");
            return convert_via_ffmpeg(src, target_fmt, output_dir);
        }
        _ => "jpeg",
    };
    let ext = if fmt == "jpeg" { "jpg" } else { fmt };

    let base = output_path_base(src, output_dir);
    ...
    let ok = match result {
        ...
        Ok(o) => {
            println!("pdftoppm failed: {}", String::from_utf8_lossy(&o.stderr));
            false
        }
        Err(e) => {
            println!("Couldn't run pdftoppm ({e}); install poppler-utils.");
            false
        }
    };

    if ok {
        // pdftoppm writes <base>-1.<ext>, <base>-2.<ext>, ... — collect them all.
        ...
        if !pages.is_empty() {
            println!(
                "PDF converted to {} {} image(s) (one per page) in {}:",
                pages.len(),
                ext,
                dir.display()
            );
    ...
    // Fallback: ffmpeg grabs a single frame.
    if has_ffmpeg() {
        println!("Falling back to ffmpeg (single image)...");
        return convert_via_ffmpeg(src, target_fmt, output_dir);
    }
    println!("PDF to image conversion requires poppler-utils (pdftoppm) or ffmpeg.");
    None
}
// ===========================================================================
